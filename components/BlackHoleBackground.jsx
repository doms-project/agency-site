  ...
  useEffect(() => {
    ...
    const initAnimation = () => {
      ...
      const scriptLines = [
        ...
        "// Wider FOV on mobile for better black hole visibility",
        "const cameraFOV = isMobileDevice ? 70 : 60;",
        "const camera = new THREE.PerspectiveCamera(cameraFOV, window.innerWidth / window.innerHeight, 0.1, 4000);",
        "// Mobile: Pull camera further back for wider view of entire black hole",
        "const aspectRatio = window.innerWidth / window.innerHeight;",
        "if (isMobileDevice) {",
        "  // Further back and adjusted angle for full black hole visibility",
        "  camera.position.set(-10.0, 5.0, 10.0);",
        "} else {",
        ...
        "    const newFOV = isMobileNow ? 70 : 60;",
        "    camera.fov = newFOV;",
        "    // Mobile: Further back for wider view",
        "    if (isMobileNow) {",
        "      camera.position.set(-10.0, 5.0, 10.0);",
        "    } else {",
        "      camera.position.set(-6.5, 6.0, 6.5);",
        ...
        "const baseCameraPos = isMobile ? new THREE.Vector3(-10.0, 5.0, 10.0) : new THREE.Vector3(-6.5, 6.0, 6.5);",
        "const blackHoleCenter = new THREE.Vector3(0, 0.5, 0);",
        "const maxZoomDistance = 2.5;",
        "",
        "const handleScroll = () => {",
        "  // Only enable zoom on desktop - keep static on mobile for better performance",
        "  if (isMobile) {",
        "    targetZoom = 0; // No zoom on mobile",
        "    return;",
        "  }",
        ...
        "",
        "  // Zoom effect - only on desktop, static on mobile",
        "  if (!isMobile) {",
        "    // Synchronized smooth interpolation",
        ...
        "    // Use same interpolation speed for synchronized movement",
        "    camera.position.lerp(newCameraPos, interpolationSpeed);",
        "  }",
        "  // Mobile: Camera stays at fixed position, no zoom",
        "",
        "  // Teleport effect",
        "  const teleportIntensity = currentZoom * 0.8;",
        "  lensingPass.uniforms.teleportIntensity.value = teleportIntensity;",
        "  // Bloom intensity",
        "  const baseBloom = quality === 'high' ? 1.0 : 0.8;",
        "  const enhancedBloom = baseBloom + teleportIntensity * (quality === 'high' ? 0.5 : 0.4);",
