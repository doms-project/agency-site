  ...
  useEffect(() => {
    ...
    const initAnimation = () => {
      ...
      const scriptLines = [
        ...
        "// Improved quality detection - favor high quality on modern devices",
        ...
        "// Use high quality by default, only use medium for very low-end devices",
        "const quality = isLowEnd ? 'medium' : 'high';",
        ...
        "// Adjust camera position for mobile portrait to ensure full animation visibility while maintaining same view",
        "const aspectRatio = window.innerWidth / window.innerHeight;",
        "const isPortrait = aspectRatio < 1;",
        "if (isMobileDevice && isPortrait) {",
        "  // Move camera further back to show full animation in portrait while keeping same FOV",
        ...
        "// Increased pixel ratio for better quality on all devices",
        ...
        "// Increased star count for richer starfield",
        "const starCount = quality === 'high' ? 200000 : quality === 'medium' ? 100000 : 30000;",
        ...
        "  fragmentShader: 'uniform float uTime; uniform vec3 uCameraPosition; varying vec3 vNormal; varying vec3 vPosition; void main() { vec3 viewDirection = normalize(uCameraPosition - vPosition); float fresnel = 1.0 - abs(dot(vNormal, viewDirection)); fresnel = pow(fresnel, 2.5); vec3 glowColor = vec3(0.518, 0.765, 0.380); float pulse = sin(uTime * 2.5) * 0.15 + 0.85; gl_FragColor = vec4(glowColor * fresnel * pulse, fresnel * 0.4); }',",
        "  transparent: true,",
        "  blending: THREE.AdditiveBlending,",
        "  side: THREE.BackSide",
        "});",
        ...
        "let targetZoom = 0;",
        "let currentZoom = 0;",
        "// Base camera position for zoom calculations - same for mobile and desktop",
        "const baseCameraPos = new THREE.Vector3(-6.5, 6.0, 6.5);",
        "const blackHoleCenter = new THREE.Vector3(0, 1.0, 0);",
        ...
        "  const heroHeight = heroSection.offsetHeight;",
        "  const scrollProgress = Math.min(scrollY / heroHeight, 1.0);",
        "  // Same zoom intensity for mobile and desktop",
        ...
        "  if (!isVisible) return;",
        "",
        "  // Frame rate limiting - more aggressive on mobile for smooth performance",
        "  const now = performance.now();",
        "  const elapsed = now - lastFrameTime;",
        "  // Mobile: 30fps (33ms), Desktop: 60fps (16ms)",
        "  const targetFrameInterval = isMobile ? 33 : frameInterval;",
        ...
        "",
        "  // Smooth zoom interpolation - slower on mobile for smoother performance",
        ...
        "  camera.position.lerp(newCameraPos, lerpSpeed);",
        "",
        "  // Teleport effect - reduced intensity on mobile for better performance",
        "  const teleportIntensity = currentZoom * (isMobile ? 0.5 : 0.8);",
        "  lensingPass.uniforms.teleportIntensity.value = teleportIntensity;",
        "  // Bloom intensity - reduced on mobile",
        "  const baseBloom = quality === 'high' ? (isMobile ? 0.7 : 1.0) : 0.8;",
        "  const bloomMultiplier = isMobile ? 0.3 : (quality === 'high' ? 0.5 : 0.4);",
        ...
        "  stars.rotation.x += deltaTime * 0.0008;",
        "  accretionDisk.rotation.z += deltaTime * 0.004;",
