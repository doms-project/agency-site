...
export async function createContact(data) {
  ...
  const surveyFieldDefinitions = [
    ...
    { surveyKey: 'marketingBudget', displayName: `${FIELD_PREFIX}Marketing_Budget`, type: 'TEXT' },
    
    // Lead Capture Form specific fields (pricing form replacement)
    // These use the exact GHL field keys created by the user: web_pricing_form_*
    { surveyKey: 'pricingWebsiteGoals', displayName: 'Web_Pricing_Form_Website_Goals', type: 'TEXT' },
    { surveyKey: 'pricingDomainName', displayName: 'Web_Pricing_Form_Domain_Name', type: 'TEXT' },
  ...
  // Add scheduling fields for strategy calls
  if (data.preferredDate) {
    customFieldsArray.push({
      key: `${FIELD_PREFIX.toLowerCase()}preferred_date`,
      field_value: data.preferredDate
    })
    console.log(`   ✅ preferred_date = "${data.preferredDate}"`)
  }

  if (data.preferredTime) {
    customFieldsArray.push({
      key: `${FIELD_PREFIX.toLowerCase()}preferred_time`,
      field_value: data.preferredTime
    })
    console.log(`   ✅ preferred_time = "${data.preferredTime}"`)
  }

  if (data.timezone) {
    customFieldsArray.push({
      key: `${FIELD_PREFIX.toLowerCase()}timezone`,
      field_value: data.timezone
    })
    console.log(`   ✅ timezone = "${data.timezone}"`)
  }

  // Create combined datetime field for easy viewing
  if (data.preferredDate && data.preferredTime) {
    const scheduledDateTime = `${data.preferredDate} ${data.preferredTime} (${data.timezone || 'America/New_York'})`
    customFieldsArray.push({
      key: `${FIELD_PREFIX.toLowerCase()}scheduled_datetime`,
      field_value: scheduledDateTime
    })
    console.log(`   ✅ scheduled_datetime = "${scheduledDateTime}"`)
  }
...
async function createOpportunityWithIds(contactId, data, fileUrls, pipelineId, stageId) {
  ...
  const isLeadCapture = source === 'Lead Capture Form (Pricing replacement)' || source.includes('Lead Capture')
  ...
  const allFieldDefinitions = [
    ...
    { surveyKey: 'gsPreferredDate', displayName: `${FIELD_PREFIX}GS_Preferred_Date`, type: 'TEXT' },
    { surveyKey: 'gsPreferredTime', displayName: `${FIELD_PREFIX}GS_Preferred_Time`, type: 'TEXT' },
    { surveyKey: 'gsTimezone', displayName: `${FIELD_PREFIX}GS_Timezone`, type: 'TEXT' },
    { surveyKey: 'gsBookingStatus', displayName: `${FIELD_PREFIX}GS_Booking_Status`, type: 'TEXT' },
  ...
  } else if (isLeadCapture) {
    notes = `Growth Program Lead (Pricing Page)\n\nName: ${data.ownerContact || 'N/A'}\nEmail: ${data.email || 'N/A'}\nPhone: ${data.phone || 'N/A'}\nBusiness Name: ${data.businessName || 'N/A'}\nWebsite: ${data.websiteUrl || 'N/A'}\n\nPrimary Goal: ${data.websiteGoals || 'N/A'}\nMarketing Budget: ${data.marketingBudget || 'N/A'}\n\nMessage:\n${data.message || 'N/A'}`
  } else {
  ...
  const opportunityName = isWebsiteRevision 
    ...
    : isSEOServices
    ? `SEO Services - ${data.businessName || 'New Lead'}`
    : isGoogleBusinessProfile
    ? `Google Business Profile - ${data.businessName || 'New Lead'}`
    : isLeadGeneration
    ? `Lead Generation - ${data.businessName || 'New Lead'}`
    : isLeadCapture
    ? `${data.websiteGoals || 'Growth Program Lead'} - ${data.businessName || data.ownerContact || 'New Lead'} - Lead`
...
export async function updateOpportunityCustomFields(opportunityId, businessName, status) {
  console.log('🔍 DEBUG: Checking GHL credentials...')
  console.log('🔍 DEBUG: GHL_API_KEY exists:', !!GHL_API_KEY)
  console.log('🔍 DEBUG: GHL_API_KEY length:', GHL_API_KEY?.length || 'undefined')
  console.log('🔍 DEBUG: GHL_LOCATION_ID exists:', !!GHL_LOCATION_ID)
  console.log('🔍 DEBUG: GHL_LOCATION_ID value:', GHL_LOCATION_ID || 'undefined')

  if (!GHL_API_KEY || !GHL_LOCATION_ID) {
    console.error('❌ GHL credentials check failed')
    throw new Error('GoHighLevel API credentials not configured')
  }

  console.log('✅ GHL credentials validated, proceeding with API call...')
  ...
  try {
    // Create emoji-based name based on status
    ...
    const cleanBusinessName = businessName ? businessName.trim() : 'New Lead'
    const opportunityName = `${emoji} Strategy Call - ${cleanBusinessName}`

    const payload = {
      name: opportunityName,
      status: "open", // Keep opportunity status as open
      monetaryValue: 0,
      customFields: [
        {
          id: 'zA7HqKLillsm8pGrsdmD',
          key: 'web_form_gs_booking_status',
          field_value: status
        }
      ]
    }

    console.log('📤 Making GHL API call to update opportunity...')
    console.log('   Opportunity ID:', opportunityId)
    console.log('   Business Name:', cleanBusinessName)
    console.log('   Status:', status)
    console.log('   New Name:', opportunityName)
    console.log('   Endpoint:', `${GHL_API_URL}/opportunities/${opportunityId}?locationId=${GHL_LOCATION_ID}`)
    console.log('   Payload:', JSON.stringify(payload, null, 2))

    const response = await fetch(
      `${GHL_API_URL}/opportunities/${opportunityId}?locationId=${GHL_LOCATION_ID}`,
      {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${GHL_API_KEY}`,
    ...
    if (!response.ok) {
      const errorData = await response.text()
      console.error('❌ GHL API Error Response:', errorData)
      throw new Error(`GHL API error: ${response.status} - ${errorData}`)
    }

    const result = await response.json()
    console.log('✅ GHL API Success Response:', JSON.stringify(result, null, 2))
    return result.opportunity || result
  } catch (error) {
    console.error('Error updating opportunity custom fields in GHL:', error)
    throw error
  }
}

/**
 * Update opportunity schedule (date and time) custom fields in GoHighLevel
 * @param {string} opportunityId - The opportunity ID
 * @param {string} newDate - New preferred date (YYYY-MM-DD format)
 * @param {string} newTime - New preferred time (HH:MM format)
 * @returns {Promise<Object>} - Updated opportunity
 */
export async function updateOpportunitySchedule(opportunityId, newDate, newTime) {
  console.log('🔍 DEBUG: Checking GHL credentials for schedule update...')
  console.log('🔍 DEBUG: GHL_API_KEY exists:', !!GHL_API_KEY)
  console.log('🔍 DEBUG: GHL_API_KEY length:', GHL_API_KEY?.length || 'undefined')
  console.log('🔍 DEBUG: GHL_LOCATION_ID exists:', !!GHL_LOCATION_ID)
  console.log('🔍 DEBUG: GHL_LOCATION_ID value:', GHL_LOCATION_ID || 'undefined')
  ...
  console.log('✅ GHL credentials validated, proceeding with schedule update...')
  ...
  try {
    // Custom field IDs for date and time (from GHL setup)
    const DATE_FIELD_ID = 'ZRYva72lDtQRfsNMIU3s' // preferred_date field
    const TIME_FIELD_ID = 'ummuIBRoOGI4I9jvMhhC' // preferred_time field
    ...
    const payload = {
      ...
      customFields: [
        {
          id: DATE_FIELD_ID,
          key: 'web_form_gs_preferred_date',
          field_value: newDate
        },
        {
          id: TIME_FIELD_ID,
          key: 'web_form_gs_preferred_time',
          field_value: newTime
        }
      ]
    }

    console.log('📤 Making GHL API call to update opportunity schedule...')
    console.log('   Opportunity ID:', opportunityId)
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error('❌ GHL API call failed:', response.status, errorText)
      throw new Error(`GHL API error: ${response.status} - ${errorText}`)
    }

    const result = await response.json()
    console.log('✅ GHL opportunity schedule updated successfully!')
    console.log('   Response:', JSON.stringify(result, null, 2))

    return result.opportunity || result
  } catch (error) {
    console.error('Error updating opportunity schedule in GHL:', error)
    throw error
  }
}
